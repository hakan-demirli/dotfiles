...
def GenerateDummyWindow(callback, uid):
    ...
    x_position = ctypes.windll.user32.GetSystemMetrics(SM_CXVIRTUALSCREEN)
    y_position = ctypes.windll.user32.GetSystemMetrics(SM_CYVIRTUALSCREEN)
...
class NotificationIcon(object):
    ...
    def _run(self):
        ...
        iconinfo.uID = 100
        iconinfo.uFlags = (
            NIF_ICON | NIF_SHOWTIP | NIF_MESSAGE | (NIF_TIP if self._tooltip else 0)
        )
    ...
    def _callback(self, hWnd, msg, wParam, lParam):
        ...
        if msg == WM_TIMER:
            if not any(
                thread.name == "MainThread" and thread.is_alive()
...
def quitsystray():
    ...
    try:
        ...
        try:
            Popen(f"taskkill /F /PID {os.getpid()} /T")
        except Exception:
            pass
...
class TrayMenu:
    def __init__(self, icon, functions):
        self.ni = NotificationIcon(icon)
        self.functions = (
            list(functions) if not isinstance(functions, list) else functions
        )
        self.ni.items = self.functions

    def announce(self, message):
        self.ni.announce(str(message))
