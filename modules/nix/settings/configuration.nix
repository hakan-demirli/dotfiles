{
  flake.modules.nixos.nix-settings = { config, pkgs, lib, ... }: {
    options.nix.custom = {
    ...
    config = {
      nixpkgs.config = {
        allowUnfree = config.nix.custom.allowUnfree;
        cudaSupport = config.nix.custom.cudaSupport;
        rocmSupport = config.nix.custom.rocmSupport;
