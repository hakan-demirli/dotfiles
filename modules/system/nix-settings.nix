...
in
{
  ...
  config = {
    assertions = [
      {
        assertion =
          !cfg.hasTailscaleAuthority
          || lib.any (lib.hasInfix "100.64.0.1:5101") config.nix.settings.substituters;
        message = "nix.custom.hasTailscaleAuthority=true requires the tailnet cache substituter.";
      }
      {
        assertion =
          !cfg.hasTailscaleAuthority
