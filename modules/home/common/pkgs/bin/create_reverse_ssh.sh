...
GIT_REPO_PATH="${HOME}/.local/share/serveo"
...
command -v ssh > /dev/null 2>&1 || {
  echo >&2 "ERROR: 'ssh' command not found."
  exit 1
}
...
command -v grep > /dev/null 2>&1 || {
  echo >&2 "ERROR: 'grep' command not found."
  exit 1
}
command -v tee > /dev/null 2>&1 || {
  echo >&2 "ERROR: 'tee' command not found."
  exit 1
}
command -v stdbuf > /dev/null 2>&1 || {
  echo >&2 "ERROR: 'stdbuf' command not found."
  exit 1
}
command -v head > /dev/null 2>&1 || {
  echo >&2 "ERROR: 'head' command not found."
  exit 1
}
...
stdbuf -oL ssh \
  ...
  | while IFS= read -r line; do
    ...
    if [[ $line == *"Forwarding SSH from "* && $line == *".$SSH_USER_HOST"* ]]; then
      ...
      escaped_host="${SSH_USER_HOST//./\\.}"
      TUNNEL_INFO=$(echo "$line" | grep -oE "[a-zA-Z0-9.-]+\.${escaped_host}")
