...
((window, fn) => {
  fn = (a, ipse, haia, hca, rpo, et) => {
    if (
      (a = document.scripts[document.scripts.length - 1]) &&
      a.id === "dyvaUjs"
    )
      a.remove();

    et = window.InstallTrigger ? "beforescriptexecute" : "message";
    ...
    window.fetch = function (u) {
      if (u) {
        if (u.substr && /\/v1\/player\/ad_break/.test(u))
          return new Promise(() => {});
        if (u.url && u.url.substr && /\/v1\/player\/ad_break/.test(u.url))
          return new Promise(() => {});
      }
    ...
    window.XMLHttpRequest.prototype.open_dyva =
      window.XMLHttpRequest.prototype.open;
    window.XMLHttpRequest.prototype.open = function (mtd, url) {
      if (
        !/get_midroll_info/.test(url) &&
        !(/^\/watch/.test(location.pathname) && /get_video_info/.test(url))
      ) {
        this.url_dyva = url;
        return this.open_dyva.apply(this, arguments);
      }
    };
    window.XMLHttpRequest.prototype.addEventListener_dyva =
    ...
    window.XMLHttpRequest.prototype.addEventListener = function (typ, fn) {
      if (typ === "readystatechange") {
        ...
        fn = function () {
          ...
          if (
            this.readyState === 4 &&
            /\/watch\?|get_video_info/.test(this.url_dyva)
          ) {
            ...
            try {
              rpo.forEach((p) => {
                if (
                  p.player &&
                  p.player.args &&
                  p.player.args.player_response
                ) {
                  p.playerResponse_ = JSON.parse_dyva(p.player_response);
                  if (
                    p.playerResponse_.playabilityStatus &&
                    p.playerResponse_.playabilityStatus.status ===
                      "LOGIN_REQUIRED"
                  ) {
                    nav.navigate(
                      {
    ...
    window.Node.prototype.appendChild = function (node) {
      ...
      if (
        !ipse &&
        (a = document.querySelector("ytd-watch-flexy")) &&
        (a = a.constructor.prototype) &&
        a.isPlaShelfEnabled_
      ) {
      ...
      if (
        ...
        Array.from(node.childNodes).some((n, i) => {
          if (n.id === "masthead-ad") {
      ...
      if (node.querySelector && (a = node.querySelector(".ytp-ad-skip-button")))
        a.click();
    ...
    function patchPlayerResponse(playerResponse) {
      ...
      if (
        playerResponse.auxiliaryUi?.messageRenderers
        ...
        if (!Object.keys(playerResponse.auxiliaryUi.messageRenderers).length) {
          delete playerResponse.auxiliaryUi.messageRenderers;
          if (!Object.keys(playerResponse.auxiliaryUi).length)
            delete playerResponse.auxiliaryUi;
        }
    ...
    function do1(ev, a) {
      if (
        (a = document.scripts[document.scripts.length - 1]) &&
        /"adPlacements"/.test(a.text)
      ) {
        a.text = a.text.replace(
          /"adPlacements"/,
          '"adPlacements":[],"zadPlacements"',
        );
      }
      if (window.loadDataHook) {
        if (!window.loadDataHook.dyva) {
          ...
          window.loadDataHook = function (ep, dt) {
            if (
              dt.playabilityStatus &&
              dt.playabilityStatus === "LOGIN_REQUIRED"
            ) {
          ...
          window.ytcfg.set = function (ytConfig, ytValue) {
            if (window.ytInitialPlayerResponse) {
              if (
                ...
                ytInitialPlayerResponse.playabilityStatus === "LOGIN_REQUIRED"
              ) {
                location.href = location.href;
                throw "Ain't gonna login";
              }
            ...
            if (ytConfig) {
              ...
              if ((a = ytConfig.EXPERIMENT_FLAGS)) {
                if (a.enable_auto_play_param_fix_for_masthead_ad)
                  a.enable_auto_play_param_fix_for_masthead_ad = false;
                if (a.html5_check_both_ad_active_and_ad_info)
                  a.html5_check_both_ad_active_and_ad_info = false;
                if (a.web_enable_ad_signals_in_it_context)
                  a.web_enable_ad_signals_in_it_context = false;
              }
              if (ytConfig.SKIP_RELATED_ADS === false)
                ytConfig.SKIP_RELATED_ADS = true;
              if (ytConfig.TIMING_ACTION)
                ytConfig.TIMING_ACTION = ytConfig.TIMING_ACTION.replace(
                  /watch7ad/,
                  "watch7",
                );
              ...
              if (
                (a = ytConfig.WEB_PLAYER_CONTEXT_CONFIGS) &&
                (a = a.WEB_PLAYER_CONTEXT_CONFIG_ID_KEVLAR_WATCH) &&
                a.serializedExperimentFlags &&
                a.serializedExperimentFlags.replace
              ) {
                a.serializedExperimentFlags =
                  a.serializedExperimentFlags.replace(
                    /([a-z][^=]+)=([^&]+)/g,
                    (s, a, b) => {
                      switch (a) {
                        ...
                        case "html5_check_both_ad_active_and_ad_info":
                          b = false;
                          break;
                        case "web_enable_ad_signals_in_it_context":
                        case "web_player_gvi_wexit_adunit":
                          b = false;
                          break;
                      }
                      return a + "=" + b;
        ...
        if (window.yt.player && window.yt.player.Application) {
            ...
            if (!window.yt.player.Application.create.dyva) {
              ...
              window.yt.player.Application.create = function (
                ...
                if (
                  ...
                  if (ytPlayerConfig.args.raw_player_response)
                    patchPlayerResponse(
                      ytPlayerConfig.args.raw_player_response,
                    );
          ...
          if (window.yt.player.Application.createAlternate) {
            if (!window.yt.player.Application.createAlternate.dyva) {
              var ytPlayerApplicationCreateAlternate =
                window.yt.player.Application.createAlternate;
              window.yt.player.Application.createAlternate = function (
                id,
                ytPlayerConfig,
              ) {
                if (
                ...
                return ytPlayerApplicationCreateAlternate.apply(
                  this,
                  arguments,
                );
              };
    ...
    addEventListener(
      ...
      function (ev) {
        if (
          ...
          ev.detail.part &&
          ev.detail.part.data &&
          ev.detail.part.data.swfcfg &&
          ev.detail.part.data.swfcfg.args
        ) {
...
.ytp-autohide .caption-window.ytp-caption-window-bottom, .ytp-hide-controls .caption-window.ytp-caption-window-bottom{margin-bottom:0!important}`;
    ...
    document.createElement = function (tag) {
      if (
        ...
        ytInitialPlayerResponse.adPlacements.length
      ) {
        patchPlayerResponse(window.ytInitialPlayerResponse);
      }
  ...
  if (
    "object" === typeof GM_info &&
    ["FireMonkey", "Greasemonkey"].includes(GM_info.scriptHandler)
  ) {
