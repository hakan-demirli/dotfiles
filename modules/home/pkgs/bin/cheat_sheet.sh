...
linuxNotesPath="${CHEATSHEETLINUXNOTESPATH:-$HOME/Desktop/infra/notes/notes/work/linux/}"
...
while true; do
  ...
  if [[ $selected_clean == "linux" ]]; then
    ...
    linux_selection=$(echo "$linux_lines_for_fzf" \
      | fzf --prompt="Search Linux Notes (file:line:content) > " --delimiter=":" --preview="echo {}")
